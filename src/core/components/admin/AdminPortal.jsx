  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-4 py-1.5 bg-slate-800 hover:bg-slate-700 text-white text-xs font-semibold rounded-xl border border-slate-700 transition ml-2 inline-flex items-center gap-1.5"
            >
        ...
        {error && (
          <div className="p-4 bg-red-900/30 border border-red-500/50 text-red-300 text-xs rounded-2xl flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-900/90 p-5 rounded-2xl border border-slate-800 shadow-lg">
              ...
              <span className="text-[11px] text-emerald-400 mt-1 flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3 shrink-0" /> {stats.signedProposals} proposals signed &amp; accepted
              </span>
            </div>
          </div>
        )}
                    ...
                    return (
                      <tr key={u.id} className={`hover:bg-slate-800/30 ${isSuspended ? 'bg-red-950/20' : isLocked ? 'bg-amber-950/20' : ''}`}>
                        ...
                        <td className="py-3 px-4 text-center">
                          ...
                          >
                            {u.has_unlimited_bypass ? (
                              <span className="inline-flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Active</span>
                            ) : 'Disabled'}
                          </button>
                        </td>
                        ...
                        <td className="py-3 px-4 text-right">
                          <div className="flex items-center justify-end gap-1.5 flex-wrap">
                            ...
                            >
                              {isTest ? <span className="inline-flex items-center gap-1"><FlaskConical className="w-3 h-3" /> Test</span> : 'Live'}
                            </button>
                            <button
                              type="button"
                              title="Dispatch Password Reset Link"
                              onClick={() => handleResetPassword(u)}
                              className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg text-xs font-semibold transition inline-flex items-center gap-1"
                            >
                            ...
                            {isLocked && (
                              <button
                                ...
                                className="px-2 py-1 bg-amber-900/40 hover:bg-amber-900/60 text-amber-300 border border-amber-500/50 rounded-lg text-xs font-semibold transition inline-flex items-center gap-1"
                              >
        ...
        {activeTab === 'promos' && (
          <div className="bg-slate-900/90 rounded-3xl border border-slate-800 p-6 space-y-6 shadow-xl">
            ...
            {promoSuccessMsg && (
              <div className="p-3 bg-emerald-900/30 border border-emerald-500/50 text-emerald-300 text-xs rounded-xl flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 shrink-0" /> {promoSuccessMsg}
              </div>
            )}
