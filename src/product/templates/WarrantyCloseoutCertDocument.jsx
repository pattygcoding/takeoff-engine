    ...
    <div className="space-y-6 font-serif text-center py-4">
      <div className="border-4 border-double border-amber-600 p-8 rounded-3xl bg-amber-50/20 space-y-5">
        ...
        <p className="text-xs text-slate-600 font-sans max-w-lg mx-auto leading-relaxed">
          {t('product.templates.warrantyCloseout.certIntro', {
            projectName: currentProject?.name || t('product.templates.warrantyCloseout.defaultProjectName')
          })}
        </p>
        ...
        <div className="grid grid-cols-2 gap-4 max-w-md mx-auto text-xs font-sans text-left bg-white p-4 rounded-xl border border-amber-200 shadow-2xs">
          <div>
            <span className="text-slate-400 font-bold">{t('product.templates.warrantyCloseout.contractorLabel')}</span>
            <p className="font-bold text-slate-900">{branding?.companyName || t('product.templates.warrantyCloseout.defaultContractor')}</p>
          </div>
          <div>
            <span className="text-slate-400 font-bold">{t('product.templates.warrantyCloseout.clientLabel')}</span>
            <p className="font-bold text-slate-900">{currentProject?.client_name || t('product.templates.warrantyCloseout.defaultClient')}</p>
          </div>
          <div>
            <span className="text-slate-400 font-bold">{t('product.templates.warrantyCloseout.warrantyPeriodLabel')}</span>
            <p className="font-bold text-emerald-700">{t('product.templates.warrantyCloseout.warrantyPeriodValue')}</p>
          </div>
          <div>
            <span className="text-slate-400 font-bold">{t('product.templates.warrantyCloseout.certifiedValueLabel')}</span>
        ...
        <p className="text-[11px] text-slate-500 font-sans italic max-w-md mx-auto">
          {t('product.templates.warrantyCloseout.warrantyTerms')}
        </p>
