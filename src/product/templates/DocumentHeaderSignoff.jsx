  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
      <div className="space-y-1">
        ...
        {branding?.companyPhone && (
          <p className="text-xs text-slate-500">
            {t('product.templates.header.phonePrefix', { phone: branding.companyPhone })}
          </p>
        )}
      </div>
      ...
      <div className="text-left sm:text-right space-y-1">
        ...
        {project?.client_name && (
          <p className="text-xs text-slate-500">
            {t('product.templates.header.clientPrefix', { client: project.client_name })}
          </p>
        )}
        {project?.location && (
          <p className="text-xs text-slate-500">
            {t('product.templates.header.sitePrefix', { site: project.location })}
          </p>
        )}
        <p className="text-[11px] font-mono text-slate-400">
          {t('product.templates.header.datePrefix', { date: new Date().toLocaleDateString() })}
        </p>
      </div>
    </div>
  );
      ...
      {clientSignBlock ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
          <div className="space-y-8">
            <span className="text-xs font-bold text-slate-700">{t('product.templates.signOff.submittedByContractor')}</span>
          ...
          <div className="space-y-8">
            <span className="text-xs font-bold text-slate-700">{t('product.templates.signOff.acceptedByClient')}</span>
            <div className="border-b border-slate-400 pb-1 flex justify-between text-xs text-slate-600">
              <span>{t('product.templates.signOff.authorizedSignature')}</span>
              <span>{t('product.templates.signOff.date')}</span>
            </div>
        ...
        <div className="flex justify-between items-center text-[11px] text-slate-400">
          ...
          <span>{t('product.templates.signOff.confidentialData')}</span>
        </div>
      )}
    </div>
  );
