...
export default function RiskContingencyMatrixDocument({ estimate, branding, currentProject, rates }) {
  ...
  return (
    <div className="space-y-6">
      <DocumentBrandingHeader branding={branding} title={t('product.templates.riskContingency.title')} project={currentProject} />

      <div className="border border-slate-200 rounded-xl overflow-hidden">
        <table className="w-full text-left text-xs">
          <thead className="bg-rose-950 text-white">
            <tr>
              <th className="p-2.5">{t('product.templates.riskContingency.colSystem')}</th>
              <th className="p-2.5">{t('product.templates.riskContingency.colSubsurfaceProfile')}</th>
              <th className="p-2.5 text-center">{t('product.templates.riskContingency.colRiskLevel')}</th>
              <th className="p-2.5 text-right">{t('product.templates.riskContingency.colBaseDirect')}</th>
              <th className="p-2.5 text-right">
                {totals.contingencyType === 'fixed'
                  ? t('product.templates.riskContingency.colContingencyBufferFixed')
                  : t('product.templates.riskContingency.colContingencyBuffer', { percent: rates?.contingencyPct ?? rates?.contingencyPercent ?? totals.contingencyPct ?? 5 })}
              </th>
            </tr>
          </thead>
              ...
              return (
                <tr key={sys.system} className="hover:bg-slate-50">
                  ...
                  <td className="p-2.5 font-sans text-slate-600">
                    {isHigh ? t('product.templates.riskContingency.highRiskText') : t('product.templates.riskContingency.moderateRiskText')}
                  </td>
                  <td className="p-2.5 text-center">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-sans font-bold ${isHigh ? 'bg-rose-100 text-rose-800' : 'bg-emerald-100 text-emerald-800'}`}>
                      {isHigh ? t('product.templates.riskContingency.riskHigh') : t('product.templates.riskContingency.riskModerate')}
                    </span>
                  </td>
          ...
          <tfoot className="bg-slate-100 font-bold border-t border-slate-300">
            <tr>
              <td colSpan={3} className="p-2.5 font-sans text-slate-700">{t('product.templates.riskContingency.totalAllocatedReserves')}</td>
