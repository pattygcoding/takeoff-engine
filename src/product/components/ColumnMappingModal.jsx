...
export default function ColumnMappingModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-3xl w-full p-6 sm:p-7 border border-slate-200 dark:border-slate-800 max-h-[90vh] flex flex-col my-auto text-slate-900 dark:text-slate-100">
        {/* Modal Header */}
        <div className="pb-4 border-b border-slate-100 dark:border-slate-800">
          ...
          {hasConfigurationBar && (
            <div className="mt-4 pt-3.5 border-t border-slate-100/80 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {/* 1. Header Row Selector (Clean Neutral Slate card) */}
              {hasRowSelector && (
                <div className={`p-2.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-2xl flex flex-col gap-1.5 ${!hasSubTables ? 'sm:col-span-2' : ''}`}>
                  ...
                  <select
                    ...
                    className="w-full text-xs bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-2.5 py-1.5 font-semibold text-slate-800 dark:text-slate-200 shadow-xs focus:ring-2 focus:ring-slate-400 focus:outline-none"
                  >
              ...
              {hasMultipleSheets && (
                <div className="p-2.5 bg-sky-50/60 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-800 rounded-2xl flex flex-col gap-1.5 sm:col-span-2">
                  <div className="flex items-center justify-between">
                  ...
                  <select
                    ...
                    className="w-full text-xs bg-white dark:bg-slate-800 border border-sky-300 dark:border-sky-700 rounded-xl px-2.5 py-1.5 font-semibold text-sky-950 dark:text-sky-200 shadow-xs focus:ring-2 focus:ring-sky-500 focus:outline-none"
                  >
                    {sheetNames.map((s) => (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
            {targetFields.map((field) => {
              ...
              return (
                <div
                  key={field.key}
                  className="p-3 bg-slate-50/50 dark:bg-slate-800/40 flex flex-wrap items-center justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/70 transition"
                >
                  ...
                  <tbody className="divide-y divide-slate-200/60 dark:divide-slate-800 bg-white dark:bg-slate-900">
                    {previewRows.map((row, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50">
                        <td className="p-2 text-slate-700 dark:text-slate-300 font-mono text-[11px] truncate max-w-[120px]">
                          {mapping.system ? String(row[mapping.system] ?? '-') : '-'}
                        </td>
                        <td className="p-2 text-slate-700 dark:text-slate-300 font-medium truncate max-w-[180px]">
                          {mapping.item_description ? String(row[mapping.item_description] ?? '-') : '-'}
                        </td>
                        <td className="p-2 text-slate-600 dark:text-slate-400 text-[11px] truncate max-w-[120px]">
                          {mapping.size_spec ? String(row[mapping.size_spec] ?? '-') : '-'}
                        </td>
                        <td className="p-2 text-slate-900 dark:text-white font-bold font-mono">
                        ...
                        <td className="p-2 text-slate-500 dark:text-slate-400 font-mono text-[11px]">
                          {mapping.avg_depth_ft ? String(row[mapping.avg_depth_ft] ?? '0') : '0'}
                        </td>
                        <td className="p-2 text-slate-700 dark:text-slate-300 font-mono text-[11px]">
